...
If the target is a mob and the difference between it's defense rating and the attacker's weapon skill is 11 or more:
...
If the target is a mob and the difference between it's defense rating and the attacker's weapon skill is 10 or less:
...
Due to the nature of the two-roll system in World of Warcraft combat, attacks past a certain $P(Crit)$ every non-glancing auto attack will result in a critical strike. This threshold is also known as the **crit cap**, and decreasing $P(Miss)$ and $P(Dodge)$ through an increase in Weapon Skill or increasing $P(Hit)$ will increase the player's crit cap. Since we cannot decrease $P(Glance)$, we can calculate the crit cap as:
...
Where $Crit_{sup}$ is the innate crit suppresion against a level 63 mob [@magey_atktbl2019]. It is important to note, however, that due to the HS queing mechanic \@ref(hsq) effectively removing the dual wield attack penalty, the crit cap for auto attacks is not something to be worried about at current gear levels.

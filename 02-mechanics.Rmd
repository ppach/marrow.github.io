...
wepskill_tbl <- tibble::tribble(
        ...
        mutate(`Weapon Skill` = seq(300, 315, by = 1),
               ...
               `Glancing Blow Damage Increase` = (`% damage dealt by glancing blow` / 0.65)-1) %>%
...
inherit_miss <- wepskill_code %>%
        select(weapon_skill, everything(), -c(percentdmg_dealt_by_glancing, glancing_blow_damage_increase)) %>%
...
wepskillplot1 <- ggplot(wepskill_code, aes(x = weapon_skill, y = percentdmg_dealt_by_glancing)) +
        geom_line() +
        geom_point() +
        labs(x = "Weapon Skill", 

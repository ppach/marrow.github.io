...
There are two types of melee attacks: white (auto attacks) and yellow (special attacks). White attacks function exactly like white attacks, except they *cannot* glance. Each white attack made by players against enemy mobs will result in one of the following:
...
## Miss Chance, Weapon Skill, and Glancing Blows
...
P\left(Miss\right)\ =\ 0.05\ +\ \left(\left(T_{lvl}\times5\right)-Atk_{skill}\right)\times0.2
(\#eq:missprob1)
\end{equation}
...
\begin{equation}
P\left(Miss\right)\ =\ 0.05\ +\ \left(\left(T_{lvl}\times5\right)-Atk_{skill}\right)\times0.1
(\#eq:missprob2)
\end{equation}

Where $T_{lvl}$ is the target's level, and $Atk_{skill}$ is the attacker's weapon skill rating. While simple, this formula carries immense significance - it means that that by having 305 weapon skill, a player only has a 6% chance to miss an enemy mob 3 levels higher, which includes raid bosses. Conversely, a player with only 300 weapon skill will have an 8% chance to miss. This is a huge difference, especially in conjunction with the other benefits that weapon skill brings (and which will be further analyzed in the  [weapon skill](#weapon_skill) section).

It is important to note, however, that this is the behavior exhibited by wielding one one-handed weapon. If dual wielding, probability of missing an attack is calculated as:
...
Glancing blows are a type of attack that can only occur when fighting an enemy of equal or higher value, and are restricted to white attacks. In accordance with Beaza, Magey has determined the glancing blow probability to be as follows:

\begin{equation}
P\left(Glancing\right)\ =\ 0.1\ +\ \left(T_{lvl}\times5\ -\ \min\left(Atk_{lvl}\times5,\ Atk_{skill}\right)\right)\times0.2
(\#eq:glancingprob)
\end{equation}

Where $Atk_{lvl}$ is the player's level, and $Atk_{skill}$ is the player's weapon skill value. Knowing that, we can compute the probability that our white attacks will glance, and the damage penalty that that glancing blow carries:
...
```{r glancing_tbl, echo=FALSE}

knitr::kable(glancing_tbl,
             booktabs = TRUE,
...
wepskill_tbl <- tibble::tribble(
        ...
        select(`Weapon Skill`, everything(), -c(Delta, `Glancing Penalty`))
...
knitr::kable(wepskill_tbl,
             booktabs = TRUE,
             caption = "Good Title here")
```

```{r echo=FALSE}
library(ggplot2)
library(ggpubr)
ggplot(wepskill_tbl, aes(x = `Weapon Skill`, y = `Percent of White Attack`)) +
        geom_line() +
        labs(x = "Weapon Skill", 
             y = "Glancing blow damage as percent of white attack",
             title = "Weapon skill greatly reduces the glancing blow damage penalty",
             subtitle = "At 308 weapon skill, its effects saturate") +
